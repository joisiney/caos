...
exports.exampleBasicAnalysis = exampleBasicAnalysis;
exports.exampleLayerClassification = exampleLayerClassification;
exports.exampleNamingSuggestions = exampleNamingSuggestions;
exports.exampleDependencyAnalysis = exampleDependencyAnalysis;
exports.exampleCodeAnalysis = exampleCodeAnalysis;
exports.exampleCompleteWorkflow = exampleCompleteWorkflow;
exports.exampleContextBuilding = exampleContextBuilding;
...
async function exampleBasicAnalysis() {
    ...
    try {
        ...
        const aiProvider = provider_factory_1.ProviderFactory.getDefaultProvider();
        // Create analyzer suite
        const suite = (0, analyzers_1.createAnalyzerSuite)(aiProvider);
...
function exampleLayerClassification() {
    ...
    const classifier = new analyzers_1.LayerClassifier();
    const testCases = [
...
async function exampleNamingSuggestions() {
    ...
    try {
        ...
        let aiProvider;
        try {
            aiProvider = provider_factory_1.ProviderFactory.getDefaultProvider();
        }
        catch {
            console.log('No AI provider available, using rule-based naming');
        }
        const namingAnalyzer = new analyzers_1.NamingAnalyzer(aiProvider);
        const testCases = [
            { description: 'botão de confirmação', layer: 'atom' },
            { description: 'modal de edição de usuário', layer: 'molecule' },
            { description: 'header principal da aplicação', layer: 'organism' },
            { description: 'tela de depósito na carteira', layer: 'feature', context: { prefix: 'wallet' } },
            { description: 'função para formatar moeda', layer: 'util' },
        ];
        for (const testCase of testCases) {
            const suggestion = await namingAnalyzer.suggestName(testCase.description, testCase.layer, testCase.context);
...
async function exampleDependencyAnalysis() {
    console.log('\n=== Example 4: Dependency Analysis ===');
    const dependencyAnalyzer = new analyzers_1.DependencyAnalyzer();
    const testCases = [
        { description: 'modal com formulário de validação', layer: 'molecule', features: ['validation'] },
        { description: 'tela de usuário com dados da API', layer: 'feature', features: ['api', 'data'] },
        { description: 'header com navegação complexa', layer: 'organism', features: ['navigation'] },
    ];
    for (const testCase of testCases) {
        const analysis = await dependencyAnalyzer.analyzeDependencies(testCase.description, testCase.layer, testCase.features);
        console.log(`\nDescription: "${testCase.description}" (${testCase.layer})`);
...
async function exampleCodeAnalysis() {
    ...
    try {
        ...
        const codeAnalyzer = new analyzers_1.CodeAnalyzer(aiProvider);
...
async function exampleCompleteWorkflow() {
    ...
    try {
        ...
        const result = await (0, analyzers_1.performComprehensiveAnalysis)(description, aiProvider, context);
        ...
        console.log(`- Alternatives: ${result.layer.alternatives.map((alt) => alt.layer).join(', ')}`);
        console.log('\n📛 Naming Suggestions:');
        ...
        console.log(`- Files: ${result.dependencies.structure.map((f) => f.filename).join(', ')}`);
        console.log('\n📊 Metadata:');
...
async function exampleContextBuilding() {
    console.log('\n=== Example 7: Context Building ===');
    const contextBuilder = new analyzers_1.ContextBuilder();
...
//# sourceMappingURL=analyzer-usage-example.js.map