...
exports.createValidationGenerator = exports.ValidationGenerator = void 0;
/**
...
class ValidationGenerator {
     ...
     */
    async generateWithValidation(analysis, templateContext, generatedFiles, options = {
     ...
     */
    async validateGeneratedCode(files, layer) {
        const errors = [];
        const warnings = [];
        let score = 1.0;
     ...
     */
    async validateFile(fileName, content, layer) {
        const errors = [];
        const warnings = [];
        let score = 1.0;
     ...
     */
    validateLayerConventions(fileName, content, layer) {
        const errors = [];
        const warnings = [];
        let score = 1.0;
     ...
     */
    validateAtomConventions(fileName, content, errors, warnings) {
     ...
     */
    validateMoleculeConventions(fileName, content, errors, warnings) {
     ...
     */
    validateOrganismConventions(fileName, content, errors, warnings) {
     ...
     */
    validateFeatureConventions(fileName, content, errors, warnings) {
     ...
     */
    validateReactConventions(fileName, content, errors, warnings) {
     ...
     */
    validateTypeScriptConventions(fileName, content, errors, warnings) {
     ...
     * Validate imports
     */
    validateImports(fileName, content) {
        const errors = [];
        const warnings = [];
        let score = 1.0;
        const lines = content.split('\n');
     ...
     * Auto-fix errors
     */
    async autoFixErrors(files, errors, context) {
        const fixedFiles = {};
        const enhancements = [];
        for (const [fileName, content] of Object.entries(files)) {
            let fixedContent = content;
            const fileErrors = errors.filter(error => error.line !== undefined || error.message.includes(fileName));
     ...
     */
    async fixFileErrors(fileName, content, errors, context) {
        let fixedContent = content;
        const enhancements = [];
     ...
     */
    async applyAutoFix(fileName, content, error, context) {
        let fixedContent = content;
        let fixed = false;
        ...
        if (error.message.includes('Missing displayName')) {
            ...
            if (!content.includes('.displayName')) {
                fixedContent = fixedContent.replace(/export function (\w+)/, `export function $1`) + `\n\n${componentName}.displayName = '${context.namespace}';`;
                fixed = true;
            }
        }
     ...
     */
    async enhanceWithAI(files, context) {
        if (!this.aiProvider) {
     ...
     */
    async enhanceFileWithAI(fileName, content, context) {
        const prompt = `
...
RETORNE APENAS O CÓDIGO MELHORADO, SEM EXPLICAÇÕES.
     ...
     * Generate improvements based on validation results
     */
    generateImprovements(errors, warnings) {
        const improvements = [];
 ...
 */
const createValidationGenerator = (aiProvider) => {
    return new ValidationGenerator(aiProvider);
};
exports.createValidationGenerator = createValidationGenerator;
