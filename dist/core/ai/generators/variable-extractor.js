...
exports.variableExtractor = exports.VariableExtractor = void 0;
/**
...
class VariableExtractor {
     ...
     */
    async extractContext(analysis, options = {}) {
        try {
     ...
     */
    async buildTemplateContext(analysis, options) {
     ...
     */
    applyPrefix(name, layer, customPrefix) {
     ...
     */
    generateNamespace(pascalName, layer) {
        const layerSuffixes = {
            atom: 'Atom',
            molecule: 'Molecule',
            organism: 'Organism',
     ...
     */
    extractFeatures(analysis, additionalFeatures) {
        const features = new Set();
        // Extract from metadata
        if (analysis.metadata['requiredFeatures']) {
            analysis.metadata['requiredFeatures'].forEach((f) => features.add(f));
        }
        if (analysis.metadata['optionalFeatures']) {
            analysis.metadata['optionalFeatures'].forEach((f) => features.add(f));
        }
     ...
     */
    async generateImports(analysis, layer) {
        const imports = [];
     ...
     */
    generatePropsImport(componentName, layer) {
        if (!this.isReactComponent(layer)) {
            return null;
        }
     ...
     */
    generateLayerSpecificImports(layer, analysis) {
        const imports = [];
        switch (layer) {
            case 'molecule':
            case 'organism':
     ...
     */
    determineFilesToGenerate(context) {
        const files = [];
 ...
 */
exports.variableExtractor = new VariableExtractor();
