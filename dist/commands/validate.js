...
exports.setupValidateCommand = setupValidateCommand;
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
const config_1 = require("../utils/config");
function setupValidateCommand(program) {
    program
        .command('validate')
...
async function validateDirectoryStructure(config, results) {
    const requiredDirs = Object.values(config.directories);
    ...
    try {
        const tsconfig = await fs_extra_1.default.readJson('tsconfig.json');
...
async function validateRepositories(config, results) {
    const repoDir = config.directories.repositories;
    if (!await fs_extra_1.default.pathExists(repoDir))
        return;
    const files = await fs_extra_1.default.readdir(repoDir);
    const repoFiles = files.filter(f => f.endsWith('.repository.ts'));
    for (const file of repoFiles) {
        const filePath = path_1.default.join(repoDir, file);
...
async function validateNamingConventions(config, results) {
    const dirs = [
...
async function validateImports(config, results) {
    const dirs = [
        config.directories.repositories,
        config.directories.gateways,
        config.directories.models,
    ...
    for (const dir of dirs) {
        ...
        const files = await fs_extra_1.default.readdir(dir);
        for (const file of files) {
            if (file === '.gitkeep' || !file.endsWith('.ts'))
                continue;
